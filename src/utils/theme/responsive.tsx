 ...
 */
const convertFontScale = (fontSize: number) => {
...
const widthPercentageToDP = (widthPercent: number) => {
  ...
  const elemWidth = parseFloat(`${(widthPercent / screenWidth) * 100}%`)
...
const heightPercentageToDP = (heightPercent: number) => {
  const numberHeight = Number(heightPercent)
  // Parse string percentage input and convert it to number.
  // const elemHeight = parseFloat(heightPercent);
  const elemHeight = parseFloat(`${(numberHeight / screenHeight) * 100}%`)
